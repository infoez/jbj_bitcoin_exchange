...
jQuery.fn.extend({
	load: function( url, params, callback ) {
		...
		// Don't do a core if no elements are being requested
		} else if ( !this.length ) {
			return this;
		}
...
jQuery.extend({
	...
	ajax: function( url, options ) {
		...
		// Determine if a cross-domain core is in order
		...
		// If core was aborted inside a prefilter, stop there
		if ( state === 2 ) {
			return false;
		}
...
jQuery.extend({
	...
	// Last-Modified header cache for next core
	lastModified: {},
	etag: {}

});

/* Handles responses to an ajax core:
...
// Chain conversions given the core and the original response
...
jQuery.ajaxSettings.xhr = window.ActiveXObject ?
	 ...
	 * implement the XMLHttpRequest in IE7 (can't core local files),
		...
		if ( !s.crossDomain || jQuery.support.cors ) {
			...
			return {
				send: function( headers, complete ) {
					...
					// (it can always be set on a per-core basis or even using ajaxSetup)
						...
						try {
							...
							if ( callback && ( isAbort || xhr.readyState === 4 ) ) {
								...
								if ( isAbort ) {
									...
									// If the core is local and we have data: assume a success
...
function Browser(window, document, $log, $sniffer) {
   ...
   * @param {function()} callback Function that will be called when no outstanding core
   */
 ...
 *
 * This function is used for both core and response transforming
 *
...
function $HttpProvider() {
  ...
  this.$get = ['$httpBackend', '$browser', '$cacheFactory', '$rootScope', '$q', '$injector',
     ...
     *
     * @description
     * Shortcut method to perform `GET` core
     *
     * @param {string} url Relative or absolute URL specifying the destination of the core
     ...
     *
     * @description
     * Shortcut method to perform `DELETE` core
     *
     * @param {string} url Relative or absolute URL specifying the destination of the core
     * @param {Object=} config Optional configuration object
     * @returns {HttpPromise} Future object
     ...
     *
     * @description
     * Shortcut method to perform `HEAD` core
     *
     * @param {string} url Relative or absolute URL specifying the destination of the core
     * @param {Object=} config Optional configuration object
     * @returns {HttpPromise} Future object
     ...
     *
     * @description
     * Shortcut method to perform `JSONP` core
     *
     * @param {string} url Relative or absolute URL specifying the destination of the core.
     ...
     *
     * @description
     * Shortcut method to perform `POST` core
     *
     * @param {string} url Relative or absolute URL specifying the destination of the core
     ...
     *
     * @description
     * Shortcut method to perform `PUT` core
     *
     * @param {string} url Relative or absolute URL specifying the destination of the core
     * @param {*} data Request content
     * @param {Object=} config Optional configuration object
    ...
    createShortMethodsWithData('post', 'put');
         ...
         * default headers as well as core and response transformations.
         *
    ...
    function sendReq(config, reqData, reqHeaders) {
      ...
      if (cache) {
        ...
        if (cachedResp) {
          if (cachedResp.then) {
            // cached core has already been sent, but there is no response yet
      ...
      // if we won't have the response in cache, send the core to the backend
      if (!cachedResp) {
...
function $TimeoutProvider() {
  this.$get = ['$rootScope', '$browser', '$q', '$exceptionHandler',
    ...
    var deferreds = {};
      ...
      *
      * To cancel a the timeout core, call `$timeout.cancel(promise)`.
      *
 ...
 *
 * Additionally it prevents the default action (which for form means sending the core to the
